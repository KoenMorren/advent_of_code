...
p2 = (data) => {
    ...
    while (password.filter(x => !!x).length < 8) {
        ...
        [position, character] = hash.substring(5, 7).split('')
        if (isNaN(position) || parseInt(position) > 7 || password[parseInt(position)] !== undefined)
            continue;
        
        password[parseInt(position)] = hash.substring(6, 7)
    }
